...
class _CraftsmanPageState extends State<CraftsmanPage> {
  ...
  // API Endpoints - Updated for craftsman
  final String listApiUrl = 'http://127.0.0.1:8000/BusinessPartner/BusinessPartner/Craftsmen/';
  final String filterApiUrl = 'http://127.0.0.1:8000/user/BusinessPartner/filter/';
  final String createApiUrl = 'http://127.0.0.1:8000/BusinessPartner/BusinessPartner/CRAFTSMAN/create/';
  ...
  List<Map<String, dynamic>> availableFields = [
    ...
    {'key': 'gst_no', 'label': 'GST Number', 'selected': true, 'order': 7},
  ...
  Future<void> loadSavedFieldSelections() async {
    ...
    String? savedSelections = prefs.getString('craftsman_fields');
    String? savedOrder = prefs.getString('craftsman_field_order');
  ...
  Future<void> saveFieldSelections() async {
    ...
    await prefs.setString('craftsman_fields', json.encode(selections));
    
    // Save field order
    List<String> orderList = availableFields.map((f) => f['key'] as String).toList();
    await prefs.setString('craftsman_field_order', json.encode(orderList));
  }
  ...
  Future<void> loadListSettings() async {
    ...
    setState(() {
      compactRows = prefs.getBool('craftsman_compact_rows') ?? false;
      activeRowHighlighting = prefs.getBool('craftsman_active_row_highlighting') ?? false;
      modernCellColoring = prefs.getBool('craftsman_modern_cell_coloring') ?? false;
      enableView = prefs.getBool('craftsman_enable_view') ?? false;
      enableEdit = prefs.getBool('craftsman_enable_edit') ?? false;
    });
  }
  ...
  Future<void> saveListSettings({
    ...
    await prefs.setBool('craftsman_compact_rows', compactRows);
    await prefs.setBool('craftsman_active_row_highlighting', activeRowHighlighting);
    await prefs.setBool('craftsman_modern_cell_coloring', modernCellColoring);
    await prefs.setBool('craftsman_enable_view', enableView);
    await prefs.setBool('craftsman_enable_edit', enableEdit);
  }
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(width: 12),
                          Text(
                            'Personalize List Columns - Craftsmen',
                            style: TextStyle(
                              fontSize: 20,
  ...
  // Initialize default fields from availableFields
  ...
  String getFieldValue(Map<String, dynamic> craftsman, String key) {
    final value = craftsman[key];
  ...
  Future<void> loadToken() async {
    ...
    if (token == null || token!.isEmpty) {
      // Initialize with default fields even without token
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (results.isNotEmpty) {
          ...
          // Initialize with default fields if no results
          initializeDefaultFields();
        }
        ...
        // Initialize with default fields on error
        initializeDefaultFields();
        setState(() => isLoading = false);
      }
    } catch (e) {
      print('Exception: $e');
      // Initialize with default fields on exception
      initializeDefaultFields();
      setState(() => isLoading = false);
    }
  }

  // Fetch single craftsman details
  Future<void> fetchCraftsmanDetails(int id) async {
    if (token == null) return;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        setState(() {
          currentViewedCraftsman = data;
          isLoading = false;
        });
        
        // Show detail dialog with fetched data
        showCraftsmanDetailDialog();
      } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to fetch craftsman details'),
  ...
  Future<void> applyFilters() async {
    ...
    if (gstNumberController.text.isNotEmpty) {
      filterParams['gst_no'] = gstNumberController.text;
    }
  ...
  Future<void> clearFilters() async {
    filterParams.clear();
    filterParams['role'] = 'craftsman'; // Keep role filter
  ...
  void showFilterDialog() {
    ...
    gstNumberController.text = filterParams['gst_no'] ?? '';

    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Filter Craftsmen'),
          content: Container(
  ...
  // Show craftsman detail dialog using fetched data
  void showCraftsmanDetailDialog() {
    if (currentViewedCraftsman == null) return;

    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Craftsman Details'),
        content: Container(
          width: double.maxFinite,
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                ...currentViewedCraftsman!.entries.where((entry) {
                ...
                // More Detail section
                if (currentViewedCraftsman!['more_detail'] != null)
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 8),
                        ...List<Map<String, dynamic>>.from(currentViewedCraftsman!['more_detail']).asMap().entries.map((entry) {
                          int index = entry.key;
  ...
  void showCraftsmanDialog(Map<String, dynamic> craftsman, bool isEdit) {
    ...
    if (isEdit) {
      ...
      if (craftsman['more_detail'] != null && craftsman['more_detail'] is List) {
        List<Map<String, dynamic>> existingMoreDetails = 
            List<Map<String, dynamic>>.from(craftsman['more_detail']);
          ...
          return AlertDialog(
            ...
            actions: [
              if (isEdit)
                ElevatedButton(
                  onPressed: () async {
                    await updateCraftsman(editingCraftsmanId!);
                    Navigator.pop(context);
                  },
                  child: Text('Save'),
                ),
  ...
  Widget _buildTextField({
    ...
    String? value,
    required Map<String, dynamic> craftsman,
  ...
  Widget _buildFileAttachmentField({
    ...
    String? fileUrl = craftsman[field];
  ...
  Widget _buildMoreDetailsSection({
    ...
    return Column(
      ...
      children: [
        ...
        else if (!isEdit && craftsman['more_detail'] != null && craftsman['more_detail'] is List)
          ...List<Map<String, dynamic>>.from(craftsman['more_detail']).asMap().entries.map((entry) {
            int index = entry.key;
            Map<String, dynamic> detail = entry.value;
  ...
  // Show add craftsman dialog with scrollable content
  void showAddCraftsmanDialog() {
    // Clear previous controllers
    createControllers.clear();
    
    // Initialize controllers - use dynamicFields if available, otherwise use availableFields as fallback
    List<String> fieldsToUse = [];
    
    if (dynamicFields.isNotEmpty) {
      fieldsToUse = dynamicFields.where((field) => 
        field.toLowerCase() != 'role' && field != 'more_detail'
      ).toList();
    } else {
      // Fallback to availableFields keys
      fieldsToUse = availableFields
          .where((field) => 
              field['key'] != 'role' && 
              field['key'] != 'more_detail' &&
              field['key'] != 'id')
    ...
    // Initialize controllers for fields
  ...
  Future<void> createCraftsman() async {
    if (token == null) return;
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // Refresh the craftsman list
        fetchCraftsmen();
        
        // Show success message
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Craftsman created successfully!'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to create craftsman. Status: ${response.statusCode}'),
            backgroundColor: Colors.red,
          ),
        );
      ...
      print('Exception in createCraftsman: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error: $e'),
          backgroundColor: Colors.red,
  ...
  Future<void> updateCraftsman(int id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Craftsman updated successfully!'),
            backgroundColor: Colors.green,
          ),
        );
      } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to update craftsman. Status: ${response.statusCode}'),
            backgroundColor: Colors.red,
          ),
        );
      ...
      print('Exception in updateCraftsman: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error: $e'),
          backgroundColor: Colors.red,
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () => fetchCraftsmen(),
            tooltip: 'Refresh',
          ),
          Padding(
            padding: const EdgeInsets.only(right: 12),
            child: ElevatedButton.icon(
              onPressed: showAddCraftsmanDialog,
              icon: Icon(Icons.add),
              label: Text('Add New'),
            ),
          ),
                                ...
                                return DataRow(
                                  ...
                                  cells: [
                                      ...
                                      isSelected
                                          ? Row(
                                              children: [
                                                ...
                                                if (enableView)
                                                  ElevatedButton(
                                                    onPressed: () => fetchCraftsmanDetails(id),
                                                    child: Text(
                                                      'View',
                                                      style: TextStyle(fontSize: compactRows ? 11 : 13),
                                                    ),
                                                ...
                                                if (enableEdit) ...[
                                                  ...
                                                  ElevatedButton(
                                                    onPressed: () => showCraftsmanDialog(craftsman, true),
                                                    child: Text(
                                                      'Edit',
                                                      style: TextStyle(fontSize: compactRows ? 11 : 13),
                                                    ),
                                    ...
                                    ...selectedFields.map((f) {
                                      String displayValue = getFieldValue(craftsman, f['key']);
                                      ...
                                      if (f['key'] == 'map_location' && displayValue.isNotEmpty) {
                                        return DataCell(
                                          GestureDetector(
                                            onTap: () {
                                              // Open map link
                                              final url = craftsman['map_location'];
                                      ...
                                      if (f['key'] == 'more_detail') {
                                        final details = craftsman['more_detail'];
...
//   // Filter and sort variables
//   Map<String, String> filterParams = {
//     'role': 'craftsman', // Always filter by craftsman role
//   };
//   String? sortBy;
...
//   // Group By / Display Fields variables - Updated for craftsman
//   List<Map<String, dynamic>> availableFields = [
//     {'key': 'bp_code', 'label': 'BP Code', 'selected': true, 'order': 0},
//     {'key': 'business_name', 'label': 'Business Name', 'selected': true, 'order': 1},
//     {'key': 'name', 'label': 'Name', 'selected': true, 'order': 2},
...
//           // Fix: Use basename correctly
//           panAttachmentFileName = file.path.split('/').last;
//         } else {
//           gstAttachmentFile = File(file.path);
//           // Fix: Use basename correctly
//           gstAttachmentFileName = file.path.split('/').last;
//         }
//       });
//     }
//   }
