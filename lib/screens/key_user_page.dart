...
class _KeyUserPageState extends State<KeyUserPage> {
  ...
  Set<int> selectedIds = {};
  String? token;
  Map<String, dynamic>? currentViewedKeyUser;
  List<String> buyerBpCodes = [];

  // API Endpoints
  final String listApiUrl = 'http://127.0.0.1:8000/user/keyuser/list/';
  final String filterApiUrl = 'http://127.0.0.1:8000/user/users/filter/';
  final String createApiUrl = 'http://127.0.0.1:8000/user/KeyUser/registration/';
  ...
  Future<void> loadSavedFieldSelections() async {
    ...
    String? savedSelections = prefs.getString('keyuser_fields');
    String? savedOrder = prefs.getString('keyuser_field_order');
  ...
  Future<void> saveFieldSelections() async {
    ...
    await prefs.setString('keyuser_fields', json.encode(selections));
    List<String> orderList = availableFields.map((f) => f['key'] as String).toList();
    await prefs.setString('keyuser_field_order', json.encode(orderList));
  }
  ...
  Future<void> loadListSettings() async {
    ...
    setState(() {
      compactRows = prefs.getBool('keyuser_compact_rows') ?? false;
      activeRowHighlighting = prefs.getBool('keyuser_active_row_highlighting') ?? false;
      modernCellColoring = prefs.getBool('keyuser_modern_cell_coloring') ?? false;
      enableView = prefs.getBool('keyuser_enable_view') ?? true;
      enableEdit = prefs.getBool('keyuser_enable_edit') ?? true;
    });
  }
  ...
  Future<void> saveListSettings({
    ...
    await prefs.setBool('keyuser_compact_rows', compactRows);
    await prefs.setBool('keyuser_active_row_highlighting', activeRowHighlighting);
    await prefs.setBool('keyuser_modern_cell_coloring', modernCellColoring);
    await prefs.setBool('keyuser_enable_view', enableView);
    await prefs.setBool('keyuser_enable_edit', enableEdit);
  }
  ...
  String getFieldValue(Map<String, dynamic> keyUser, String key) {
    final value = keyUser[key];
  ...
  Future<void> fetchKeyUsers({String? url}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() => isLoading = false);
        _showSnackBar('Failed to fetch key users: ${response.statusCode}', isError: true);
      }
    } catch (e) {
      print('Exception: $e');
      setState(() => isLoading = false);
  ...
  // Fetch Single Key User Details
  Future<void> fetchKeyUserDetails(int id) async {
    if (token == null) return;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        setState(() {
          currentViewedKeyUser = data;
          isLoading = false;
        });
        showKeyUserDetailDialog();
      } else {
        setState(() => isLoading = false);
        _showSnackBar('Failed to fetch key user details', isError: true);
      }
    } catch (e) {
      setState(() => isLoading = false);
      _showSnackBar('Error: $e', isError: true);
  ...
  // Create Key User Methods
  void showAddKeyUserDialog() {
  ...
  Future<void> createKeyUser() async {
    if (token == null) return;
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        await fetchKeyUsers();
        _showSnackBar('Key User created successfully!');
      } else {
        print('Error response: $responseBody');
        _showSnackBar('Failed to create key user: ${response.statusCode}', isError: true);
      }
    } catch (e) {
      print('Error in createKeyUser: $e');
      _showSnackBar('Error: $e', isError: true);
    } finally {
      setState(() => isLoading = false);
    }
  ...
  // View Key User Details
  void showKeyUserDetailDialog() {
    if (currentViewedKeyUser == null) return;

    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Key User Details'),
        content: Container(
          width: double.maxFinite,
          constraints: BoxConstraints(
            maxHeight: MediaQuery.of(context).size.height * 0.7,
  ...
  Widget _buildDetailField(String field) {
    dynamic value = currentViewedKeyUser?[field];
  ...
  // Edit Key User Methods
  void showEditKeyUserDialog(Map<String, dynamic> keyUser) {
    editingKeyUserId = keyUser['id'];
    editControllers = {};

    for (var field in requiredFields) {
      if (!excludeFromEdit.contains(field) && !isFileField(field)) {
        editControllers![field] = TextEditingController(
          text: keyUser[field]?.toString() ?? '',
        );
      }
    }

    selectedEditStatus = keyUser['status'];
    showPassword = false;
        ...
        builder: (context, setState) {
          return AlertDialog(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    _buildEditFileField('profile_picture', 'Profile Picture', Icons.image, keyUser, setState),
                    _buildEditFileField('aadhar_photo', 'Aadhar Photo', Icons.credit_card, keyUser, setState),
                  ],
                ),
              ),
            ),
            actions: [
              ElevatedButton(
                onPressed: () async {
                  await updateKeyUser(editingKeyUserId!);
                  Navigator.pop(context);
                },
                child: Text('Save'),
              ),
  ...
  Widget _buildEditFileField(String field, String label, IconData icon,
      Map<String, dynamic> keyUser, StateSetter setState) {
    String? fileUrl = keyUser[field];
    XFile? currentFile = field == 'profile_picture' ? profilePictureXFile : aadharPhotoXFile;
  ...
  Future<void> updateKeyUser(int id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await fetchKeyUsers();
        _showSnackBar('Key User updated successfully!');
      } else {
        print('Error response: $responseBody');
        _showSnackBar('Failed to update key user: ${response.statusCode}', isError: true);
      }
    } catch (e) {
      print('Error in updateKeyUser: $e');
      _showSnackBar('Error: $e', isError: true);
    } finally {
      setState(() => isLoading = false);
    }
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(width: 12),
                          Text(
                            'Personalize List Columns - Key Users',
                            style: TextStyle(
                              fontSize: 20,
                              fontWeight: FontWeight.w500,
                            ),
  ...
  List<DataRow> _buildTableRows() {
    ...
    return keyUsers.map((keyUser) {
          ...
          isSelected
              ? Row(
                  ...
                  children: [
                    if (enableView)
                      ElevatedButton(
                        onPressed: () => fetchKeyUserDetails(id),
                        child: Text(
                          'View',
                          style: TextStyle(fontSize: compactRows ? 11 : 13),
                        ),
                    ...
                    if (enableEdit) ...[
                      ...
                      ElevatedButton(
                        onPressed: () => showEditKeyUserDialog(keyUser),
                        child: Text(
                          'Edit',
                          style: TextStyle(fontSize: compactRows ? 11 : 13),
                        ),
      ...
      for (var field in selectedFields) {
        String displayValue = getFieldValue(keyUser, field['key']);

        if (field['isFile'] == true && displayValue != '-') {
          cells.add(
            DataCell(
              InkWell(
                onTap: () => _showFileDialog(field['label'], keyUser[field['key']].toString()),
                child: Container(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () => fetchKeyUsers(),
            tooltip: 'Refresh',
          ),
          Padding(
            padding: const EdgeInsets.only(right: 12),
            child: ElevatedButton.icon(
              onPressed: showAddKeyUserDialog,
              icon: Icon(Icons.add),
              label: Text('Add Key User'),
              style: ElevatedButton.styleFrom(
...
// //   // API Endpoints
// //   final String listApiUrl = 'http://127.0.0.1:8000/user/keyuser/list/';
// //   final String filterApiUrl = 'http://127.0.0.1:8000/user/keyuser/filter/';
// //   final String createApiUrl = 'http://127.0.0.1:8000/user/KeyUser/registration/';
// //   final String detailApiUrl = 'http://127.0.0.1:8000/user/User/detail/';
// //   final String updateApiUrl = 'http://127.0.0.1:8000/user/KeyUser/update/';
// //   final String buyerApiUrl = 'http://127.0.0.1:8000/BusinessPartner/BusinessPartner/Buyers/';
...
// //   // Filter and sort variables
// //   Map<String, String> filterParams = {'role_name': 'keyuser'};
// //   String? sortBy;
// //   String? sortOrder;
...
// //     await loadSavedFieldSelections();
...
// //     for (var controller in createControllers.values) {
// //       controller.dispose();
// //     }
...
// //       await prefs.setString('keyuser_field_order', json.encode(orderList));
// //     } catch (e) {
// //       print('Error saving field selections: $e');
// //     }
// //   }
...
// //       });
// //     } catch (e) {
// //       print('Error loading list settings: $e');
// //     }
// //   }
...
// //       await prefs.setBool('keyuser_enable_edit', enableEdit);
// //     } catch (e) {
// //       print('Error saving list settings: $e');
// //     }
// //   }
...
// //         setState(() {
// //           buyerBpCodes = buyerList.map((buyer) {
// //             String bpCode = buyer['bp_code']?.toString() ?? '';
// //             String bpName = buyer['bp_name']?.toString() ?? '';
// //             return bpName.isNotEmpty ? '$bpCode - $bpName' : bpCode;
// //           }).toList();
// //         });
// //       }
// //     } catch (e) {
...
// //         print('Error: ${response.statusCode} - ${response.body}');
// //         setState(() => isLoading = false);
// //         _showSnackBar('Failed to fetch key users', isError: true);
// //       }
// //     } catch (e) {
// //       print('Exception: $e');
// //       setState(() => isLoading = false);
...
// //   Future<void> applyFilters() async {
// //     filterParams.clear();
// //     filterParams['role_name'] = 'keyuser';

// //     if (userCodeController.text.isNotEmpty) {
// //       filterParams['user_code'] = userCodeController.text;
// //     }
// //     if (bpCodeController.text.isNotEmpty) {
// //       filterParams['bp_code'] = bpCodeController.text;
// //     }
...
// //     userCodeController.text = filterParams['user_code'] ?? '';
// //     bpCodeController.text = filterParams['bp_code'] ?? '';
// //     fullNameController.text = filterParams['full_name'] ?? '';
// //     emailIdController.text = filterParams['email_id'] ?? '';
// //     mobileNoController.text = filterParams['mobile_no'] ?? '';
...
// //                     children: [
// //                       _buildFilterField(
// //                           userCodeController, 'User Code', Icons.code),
// //                       _buildFilterField(
// //                           bpCodeController, 'BP Code', Icons.qr_code),
// //                       _buildFilterField(
...
// //                         items: buyerBpCodes.map((bp) {
// //                           return DropdownMenuItem(
// //                             value: bp.split('-').first.trim(),
// //                             child: Text(bp),
// //                           );
// //                         }).toList(),
// //                         onChanged: (value) {
// //                           setState(() {
// //                             if (createControllers.containsKey('bp_code')) {
// //                               createControllers['bp_code']?.text = value ?? '';
// //                             }
// //                           });
// //                         },
...
// //       // Add role_name
// //       request.fields['role_name'] = 'keyuser';
...
// //         print('Error response: $responseBody');
// //         _showSnackBar('Failed to create key user', isError: true);
// //       }
// //     } catch (e) {
// //       _showSnackBar('Error: $e', isError: true);
// //     } finally {
...
// //                       margin: EdgeInsets.symmetric(vertical: 6),
// //                       child: DropdownButtonFormField<String>(
// //                         value: editControllers!['bp_code']?.text,
// //                         decoration: InputDecoration(
// //                           labelText: 'BP Code',
// //                           border: OutlineInputBorder(),
// //                           prefixIcon: Icon(Icons.qr_code),
...
// //                         onChanged: (value) {
// //                           setState(() {
// //                             if (editControllers!.containsKey('bp_code')) {
// //                               editControllers!['bp_code']?.text = value ?? '';
// //                             }
// //                           });
// //                         },
...
// //                         Icons.image, keyUser, setState),
// //                     _buildEditFileField('aadhar_photo', 'Aadhar Photo',
// //                         Icons.credit_card, keyUser, setState),
// //                   ],
// //                 ),
// //               ),
// //             ),
...
// //         final responseBody = await response.stream.bytesToString();
// //         print('Error response: $responseBody');
// //         _showSnackBar('Failed to update key user', isError: true);
// //       }
// //     } catch (e) {
// //       _showSnackBar('Error: $e', isError: true);
// //     } finally {
...
// //               Navigator.pop(context);
// //               _showSnackBar('Opening file...');
// //               // Add actual file opening logic here
// //             },
// //             child: Text('Open'),
// //           ),
// //         ],
...
// //             columnCount,
// //             (index) => DataCell(
// //               index == 1 ? const SizedBox.shrink() : const Text('No data'),
// //             ),
// //           ),
// //         ),
// //       ];
...
// //                     if (enableEdit) ...[
// //                       if (enableView) const SizedBox(width: 4),
// //                       ElevatedButton(
// //                         onPressed: () => showEditKeyUserDialog(keyUser),
// //                         child: Text(
// //                           'Edit',
...
// //                     field['label'], keyUser[field['key']].toString()),
// //                 child: Container(
...
// //                               ),
// //                               const SizedBox(width: 8),
// //                               const Text('Active row highlighting'),
// //                               const SizedBox(width: 32),
// //                               SizedBox(
// //                                 width: 24,
// //                                 height: 24,
...
// //                               ),
// //                               const SizedBox(width: 8),
// //                               const Text('Modern cell coloring'),
// //                             ],
// //                           ),
...
// //     // Show confirmation
// //     _showSnackBar('List settings applied');
// //   }
...
// //                   child: Row(
// //                     children: [
// //                       const Icon(Icons.view_column, size: 16, color: Colors.purple),
// //                       const SizedBox(width: 8),
// //                       Expanded(
// //                         child: Text(
// //                           'Showing ${getSelectedFields().length} fields',
