...
class _CraftmanPageState extends State<CraftmanPage> {
  // Data lists
  List<Map<String, dynamic>> craftmen = [];
  bool isLoading = true;
  Set<int> selectedIds = {};
  String? token;
  Map<String, dynamic>? currentViewedCraftman;
  List<String> buyerBpCodes = [];

  // API Endpoints
  final String listApiUrl = 'http://127.0.0.1:8000/user/craftsman/list/';
  final String filterApiUrl = 'http://127.0.0.1:8000/user/users/filter/';
  final String createApiUrl = 'http://127.0.0.1:8000/user/Craftsman/registration/';
  ...
  final String buyerApiUrl = 'http://127.0.0.1:8000/BusinessPartner/BusinessPartner/Craftsmans/';
  ...
  Future<void> loadSavedFieldSelections() async {
    ...
    String? savedSelections = prefs.getString('craftman_fields');
    String? savedOrder = prefs.getString('craftman_field_order');
  ...
  Future<void> saveFieldSelections() async {
    ...
    await prefs.setString('craftman_fields', json.encode(selections));
    List<String> orderList = availableFields.map((f) => f['key'] as String).toList();
    await prefs.setString('craftman_field_order', json.encode(orderList));
  }
  ...
  Future<void> loadListSettings() async {
    ...
    setState(() {
      compactRows = prefs.getBool('craftman_compact_rows') ?? false;
      activeRowHighlighting = prefs.getBool('craftman_active_row_highlighting') ?? false;
      modernCellColoring = prefs.getBool('craftman_modern_cell_coloring') ?? false;
      enableView = prefs.getBool('craftman_enable_view') ?? true;
      enableEdit = prefs.getBool('craftman_enable_edit') ?? true;
    });
  }
  ...
  Future<void> saveListSettings({
    ...
    await prefs.setBool('craftman_compact_rows', compactRows);
    await prefs.setBool('craftman_active_row_highlighting', activeRowHighlighting);
    await prefs.setBool('craftman_modern_cell_coloring', modernCellColoring);
    await prefs.setBool('craftman_enable_view', enableView);
    await prefs.setBool('craftman_enable_edit', enableEdit);
  }
  ...
  String getFieldValue(Map<String, dynamic> craftman, String key) {
    final value = craftman[key];
  ...
  // Fetch Craftmen
  Future<void> fetchCraftmen({String? url}) async {
    if (token == null) return;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() => isLoading = false);
        _showSnackBar('Failed to fetch craftmen: ${response.statusCode}', isError: true);
      }
    } catch (e) {
      print('Exception: $e');
      setState(() => isLoading = false);
  ...
  // Fetch Single Craftman Details
  Future<void> fetchCraftmanDetails(int id) async {
    if (token == null) return;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        setState(() {
          currentViewedCraftman = data;
          isLoading = false;
        });
        showCraftmanDetailDialog();
      } else {
        setState(() => isLoading = false);
        _showSnackBar('Failed to fetch craftman details', isError: true);
      }
    } catch (e) {
      setState(() => isLoading = false);
      _showSnackBar('Error: $e', isError: true);
  ...
  // Create Craftman Methods
  void showAddCraftmanDialog() {
  ...
  Future<void> createCraftman() async {
    if (token == null) return;
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        await fetchCraftmen();
        _showSnackBar('Craftman created successfully!');
      } else {
        print('Error response: $responseBody');
        _showSnackBar('Failed to create craftman: ${response.statusCode}', isError: true);
      }
    } catch (e) {
      print('Error in createCraftman: $e');
      _showSnackBar('Error: $e', isError: true);
    } finally {
      setState(() => isLoading = false);
    }
  ...
  // View Craftman Details
  void showCraftmanDetailDialog() {
    if (currentViewedCraftman == null) return;

    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Craftman Details'),
        content: Container(
          width: double.maxFinite,
          constraints: BoxConstraints(
            maxHeight: MediaQuery.of(context).size.height * 0.7,
  ...
  Widget _buildDetailField(String field) {
    dynamic value = currentViewedCraftman?[field];
  ...
  // Edit Craftman Methods
  void showEditCraftmanDialog(Map<String, dynamic> craftman) {
    editingCraftmanId = craftman['id'];
    editControllers = {};

    for (var field in requiredFields) {
      if (!excludeFromEdit.contains(field) && !isFileField(field)) {
        editControllers![field] = TextEditingController(
          text: craftman[field]?.toString() ?? '',
        );
      }
    }

    selectedEditStatus = craftman['status'];
    showPassword = false;
        ...
        builder: (context, setState) {
          return AlertDialog(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    _buildEditFileField('profile_picture', 'Profile Picture', Icons.image, craftman, setState),
                    _buildEditFileField('aadhar_photo', 'Aadhar Photo', Icons.credit_card, craftman, setState),
                  ],
                ),
              ),
            ),
            actions: [
              ElevatedButton(
                onPressed: () async {
                  await updateCraftman(editingCraftmanId!);
                  Navigator.pop(context);
                },
                child: Text('Save'),
              ),
  ...
  Widget _buildEditFileField(String field, String label, IconData icon,
      Map<String, dynamic> craftman, StateSetter setState) {
    String? fileUrl = craftman[field];
    XFile? currentFile = field == 'profile_picture' ? profilePictureXFile : aadharPhotoXFile;
  ...
  Future<void> updateCraftman(int id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await fetchCraftmen();
        _showSnackBar('Craftman updated successfully!');
      } else {
        print('Error response: $responseBody');
        _showSnackBar('Failed to update craftman: ${response.statusCode}', isError: true);
      }
    } catch (e) {
      print('Error in updateCraftman: $e');
      _showSnackBar('Error: $e', isError: true);
    } finally {
      setState(() => isLoading = false);
    }
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(width: 12),
                          Text(
                            'Personalize List Columns - Craftmen',
                            style: TextStyle(
                              fontSize: 20,
                              fontWeight: FontWeight.w500,
                            ),
  ...
  List<DataRow> _buildTableRows() {
    ...
    return craftmen.map((craftman) {
      final id = craftman['id'];
          ...
          isSelected
              ? Row(
                  ...
                  children: [
                    if (enableView)
                      ElevatedButton(
                        onPressed: () => fetchCraftmanDetails(id),
                        child: Text(
                          'View',
                          style: TextStyle(fontSize: compactRows ? 11 : 13),
                        ),
                    ...
                    if (enableEdit) ...[
                      ...
                      ElevatedButton(
                        onPressed: () => showEditCraftmanDialog(craftman),
                        child: Text(
                          'Edit',
                          style: TextStyle(fontSize: compactRows ? 11 : 13),
                        ),
      ...
      for (var field in selectedFields) {
        String displayValue = getFieldValue(craftman, field['key']);

        if (field['isFile'] == true && displayValue != '-') {
          cells.add(
            DataCell(
              InkWell(
                onTap: () => _showFileDialog(field['label'], craftman[field['key']].toString()),
                child: Container(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () => fetchCraftmen(),
            tooltip: 'Refresh',
          ),
          Padding(
            padding: const EdgeInsets.only(right: 12),
            child: ElevatedButton.icon(
              onPressed: showAddCraftmanDialog,
              icon: Icon(Icons.add),
              label: Text('Add Craftman'),
              style: ElevatedButton.styleFrom(
