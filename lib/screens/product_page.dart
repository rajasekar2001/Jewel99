...
class _ProductPageState extends State<ProductPage> {
  ...
  // Get field value with proper formatting - FIXED
  String getFieldValue(Map<String, dynamic> product, String key) {
    ...
    // Handle category and subcategory as strings directly
    if (key == 'product_category' || key == 'sub_category') {
      return value.toString();
    }
  ...
  // Get subcategory ID by name for a specific category - NEW
  int? getSubcategoryIdByName(int? categoryId, String? name) {
    if (categoryId == null || name == null) return null;
    try {
      final subcategories = categorySubcategories[categoryId] ?? [];
  ...
  void showEditProductDialog(Map<String, dynamic> product) {
    ...
    // Handle category - convert from name to ID
    final catName = product['product_category'];
    if (catName != null && catName.toString().isNotEmpty) {
      try {
        final cat = categories.firstWhere(
          (c) => c['name'] == catName.toString(),
          orElse: () => <String, dynamic>{},
        );
        if (cat.isNotEmpty) {
          selectedEditCategoryId = cat['id'];
    ...
    // Handle subcategory - will be set after subcategories are loaded or try to find it
    final subName = product['sub_category'];
    if (subName != null && subName.toString().isNotEmpty && selectedEditCategoryId != null) {
      // We'll try to find the subcategory ID, but it might not be loaded yet
      // The dropdown will handle the name display, and we'll convert to ID when saving
      selectedEditSubCategoryId = null; // Will be set when subcategories load
    }
