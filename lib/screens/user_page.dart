...
class _UserPageState extends State<UserPage> {
  ...
  Set<int> selectedIds = {};
  String? token;
  Map<String, dynamic>? currentViewedUser;
  List<String> buyerBpCodes = [];

  // API Endpoints
  final String listApiUrl = 'http://127.0.0.1:8000/user/user/list/';
  final String filterApiUrl = 'http://127.0.0.1:8000/user/users/filter/';
  final String createApiUrl = 'http://127.0.0.1:8000/user/User/registration/';
  ...
  Future<void> loadSavedFieldSelections() async {
    ...
    String? savedSelections = prefs.getString('user_fields');
    String? savedOrder = prefs.getString('user_field_order');
  ...
  Future<void> saveFieldSelections() async {
    ...
    await prefs.setString('user_fields', json.encode(selections));
    List<String> orderList = availableFields.map((f) => f['key'] as String).toList();
    await prefs.setString('user_field_order', json.encode(orderList));
  }
  ...
  Future<void> loadListSettings() async {
    ...
    setState(() {
      compactRows = prefs.getBool('user_compact_rows') ?? false;
      activeRowHighlighting = prefs.getBool('user_active_row_highlighting') ?? false;
      modernCellColoring = prefs.getBool('user_modern_cell_coloring') ?? false;
      enableView = prefs.getBool('user_enable_view') ?? true;
      enableEdit = prefs.getBool('user_enable_edit') ?? true;
    });
  }
  ...
  Future<void> saveListSettings({
    ...
    await prefs.setBool('user_compact_rows', compactRows);
    await prefs.setBool('user_active_row_highlighting', activeRowHighlighting);
    await prefs.setBool('user_modern_cell_coloring', modernCellColoring);
    await prefs.setBool('user_enable_view', enableView);
    await prefs.setBool('user_enable_edit', enableEdit);
  }
  ...
  String getFieldValue(Map<String, dynamic> user, String key) {
  ...
  Future<void> fetchUsers({String? url}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() => isLoading = false);
        _showSnackBar('Failed to fetch users: ${response.statusCode}', isError: true);
      }
    } catch (e) {
      print('Exception: $e');
      setState(() => isLoading = false);
  ...
  Future<void> fetchUserDetails(int id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() => isLoading = false);
        _showSnackBar('Failed to fetch user details', isError: true);
      }
    } catch (e) {
      setState(() => isLoading = false);
      _showSnackBar('Error: $e', isError: true);
  ...
  Future<void> createUser() async {
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        _showSnackBar('Failed to create user: ${response.statusCode}', isError: true);
      }
    } catch (e) {
  ...
  void showUserDetailDialog() {
    if (currentViewedUser == null) return;
  ...
  Widget _buildDetailField(String field) {
    dynamic value = currentViewedUser?[field];
  ...
  void showEditUserDialog(Map<String, dynamic> user) {
    ...
    selectedEditStatus = user['status'];
    showPassword = false;
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    _buildEditFileField('profile_picture', 'Profile Picture', Icons.image, user, setState),
                    _buildEditFileField('aadhar_photo', 'Aadhar Photo', Icons.credit_card, user, setState),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildEditFileField(String field, String label, IconData icon,
      Map<String, dynamic> user, StateSetter setState) {
    String? fileUrl = user[field];
    XFile? currentFile = field == 'profile_picture' ? profilePictureXFile : aadharPhotoXFile;
  ...
  Future<void> updateUser(int id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Error response: $responseBody');
        _showSnackBar('Failed to update user: ${response.statusCode}', isError: true);
      }
    } catch (e) {
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(width: 12),
                          Text(
                            'Personalize List Columns - Users',
                            style: TextStyle(
                              fontSize: 20,
                              fontWeight: FontWeight.w500,
                            ),
  ...
  List<DataRow> _buildTableRows() {
    ...
    return users.map((user) {
      ...
      for (var field in selectedFields) {
        String displayValue = getFieldValue(user, field['key']);

        if (field['isFile'] == true && displayValue != '-') {
          cells.add(
            DataCell(
              InkWell(
                onTap: () => _showFileDialog(field['label'], user[field['key']].toString()),
                child: Container(
...
//   // API Endpoints
//   final String listApiUrl = 'http://127.0.0.1:8000/user/user/list/';
//   final String filterApiUrl = 'http://127.0.0.1:8000/user/user/filter/';
//   final String createApiUrl = 'http://127.0.0.1:8000/user/User/registration/';
//   final String detailApiUrl = 'http://127.0.0.1:8000/user/User/detail/';
//   final String updateApiUrl = 'http://127.0.0.1:8000/user/User/update/';
//   final String buyerApiUrl = 'http://127.0.0.1:8000/BusinessPartner/BusinessPartner/Buyers/';
